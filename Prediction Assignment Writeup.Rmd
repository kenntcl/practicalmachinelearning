...
Finally, we will implement a Random Forest model. Upon evaluating the model, we get an accuracy of 99%. With this accuracy, we use the model to predict on the original testing data set.
...
The model will be tested using Random Forest algorithm, after which we will use the model to predict on the original testing data set.
...
Random Forest algorithm is known for their ability of detecting the features that are important for classification. Feature selection is inherent, so it is not so necessary at the data preparation phase. Thus, there won't be any feature selection section in this report.
...
The accuracy of the model using Random Forest algorithm is 0.993. The expected out-of-sample error is estimated at 0.007, or 0.7%. The expected out-of-sample error is calculated as 1 - accuracy for predictions made against the cross-validation set. Our Test data set comprises of 20 cases. With an accuracy of above 99% on our cross-validation data set, we can expect that very few or none of the test samples will be missclassified.
